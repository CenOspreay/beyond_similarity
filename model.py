...
class GMNnet(torch.nn.Module):
    ...
    def forward(self, data, mode='Train'):
        ...
        for i in range(self.num_layers):
            x1, x2, score, attn_1, attn_2 = self.gmnlayer.forward(x1, x2, edge_index1, edge_index2, edge_weight1,
                                                                  edge_weight2, mode='train')
    ...
    def encode(self, x, edge_index, edge_attr):
        # 如果传进来的是 list/ndarray，就自动转 Tensor
        ...
        # 构造 forward 需要的 data 结构
        data = [x, x, edge_index, edge_index, edge_attr, edge_attr]
        hg, *_ = self.forward(data, mode='infer')
        return hg.squeeze(0)
